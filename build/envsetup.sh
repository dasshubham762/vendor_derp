...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/derp/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Derp model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch derp_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/derp-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.derp.device | grep -q "$DERP_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $DERP_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function gerritpush()
{
    ...
    GERRIT_URL=review.derpfest.org;
    DEFAULT_BRANCH=11;
    PROJECT_PREFIX=;
    ref=for;
...
function installboot()
{
    ...
    if (adb shell getprop ro.derp.device | grep -q "$DERP_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        derpremote
        git push derp HEAD:refs/heads/'$1'
    fi
    '
}
...
function deleteOTA() {
    ...
    curl --header "Authorization: Token $OTA_API_USER_TOKEN" \
        ...
        --request DELETE \
        https://api.derpfest.org/api/ota/builds/$1/;
}
...
function pushOTA() {
    ...
    device=$(grep ro\.derp\.device $OUT/system/build.prop | cut -d= -f2);
    file=$(ls -t ${OUT}/derp_$device-10* | sed -n 2p);
    md5=$(md5sum $file | awk '{ print $1 }');
    build_type=$(echo $BUILD_TYPE | tr '[:upper:]' '[:lower:]');
    size=$(stat -c%s $file);
    version=$(grep ro\.derp\.status $OUT/system/build.prop | cut -d= -f2);
    if [ -z $version ]; then
        version=$(grep ro\.derp\.status $OUT/vendor/build.prop | cut -d= -f2)
    fi
    notes=""
    ...
    url="https://sourceforge.net/projects/derpfest/files/$device/${file##*/}";
    if [[ "${TEST_BUILD}" == true ]]; then
        url="https://sourceforge.net/projects/derpfest/files/$device/cucked/${file##*/}";
    fi
    ...
    curl --header "Authorization: Token $OTA_API_USER_TOKEN" \
        ...
        --request POST \
        --data "$data" \
        https://api.derpfest.org/api/ota/builds/;
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.derp.device | grep -q "$DERP_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/derp/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
